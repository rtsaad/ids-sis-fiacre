...
class Reader(Process): 
    ...
    def  __init__(self, periodic_time, plc_scan_cycle, q_in, q_out, log_queue):
    ...
    def run(self):
        ...
        values = opc.read_group('reader')
        vals = opc.list_variables('PLC01.Application.IOCONFIG_GLOBALS_MAPPING')

        while True:
            start_time = current_milli_time()
            # Store PLC Inputs
            table_in = opc.read_group('input')            
            #sleep(self.plc_scan_cycle)
            # Store PLC Outputs
            table_out = opc.read_group('output') 
