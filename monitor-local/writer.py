...
class Writer(Process): 
    ...
    def  __init__(self, periodic_time, opc_server, log_queue):
    ...
    def run(self): 
        ...
        self.opc_conn.update(opc.write_batch(outputs, self.opc_conn))
        reset_list = ([], [])
        for inp in opc_vars.OPC_VARIABLES_LIST:
            direction = inp[3]
            if direction == 'RESET':
            ...
            while True:
                ...
                if tics > 12:
                    ...
                    if len(reset_list[0]) > 0:
                        ...
                        self.opc_conn.update(opc.write_batch(write_off + reset_list[0], self.opc_conn))
                        time.sleep(6)
                        # Release Variables
                    ...
                    self.opc_conn.update(opc.write_batch(write, self.opc_conn))
                    tics = 0
                tics = tics + 1
                # Read OPc variables
                self.log(0,"Sleeping...")
